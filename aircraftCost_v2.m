...
Q = 90; %lesser of production quantity OR number to be produced in 5 years
...
C_av = (4000 * 657000) / 1000000000; %avionics cost (range from 5~25% of flyaway cost or $4000~$8000 per pound)
...
C_M = (22.1 * W_e^0.921 * V^0.621 * Q^0.799) / 1000000000; %Manufacturing materials cost
%C_eng = 3112 * ((0.043*T_max) + (243.25*M_max) + (0.969*T_ti) - 2228); %Engine production cost
C_eng = 0; %existing engine so we should have cost
...
RDTE_2024_total = RDTE_2024 * 3;
%display(RDTE_2024 / 1000000) % million USD (2024)

% Cost per aircraft
ac_cost = RDTE_2024 / Q; 
ac_cost_market = ac_cost * 1.1; % 10% profit margin
